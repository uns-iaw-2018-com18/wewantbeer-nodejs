...
const rate = function(req, res) {
  users.findOne({"_id": req.user._id}, {"rating": {$elemMatch: {"bar": req.body.bar}}}).exec((err, result) => {
    var valid = (err || result.rating === undefined || result.rating.length == 0); // Si todavia el usuario no puntuo la cerveceria
    if (valid) {
...
const unrate = function(req, res) {
  users.findOne({"_id": req.user._id}, {"rating": {$elemMatch: {"bar": req.body.bar}}}).exec((err, result) => {
    var invalid = (err || result.rating === undefined || result.rating.length == 0); // Si todavia el usuario no despuntuo la cerveceria
    if (!invalid) {
      var actualValue = result.rating[0].count == req.body.count; // Valor real de puntaje en la base de datos
      if (actualValue) {
        // Actualizar cerveceria
        Cervecerias.findOne({"id": req.body.bar}).exec((err, cerveceria) => {
          var puntaje, cantidad;
